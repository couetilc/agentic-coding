# .agent/ — {{PROJECT}} agent-container config
...
The shims pin the major: `npx -y --package @couetilc/agentic-coding@^{{MAJOR}} -- agent ...`. Patch
...
`agent init` to rewrite the shims to the new major (`@^{{MAJOR}}` → the next).
